...
def _safe_choice_from_probs(probs: torch.Tensor) -> int:
    safe_probs = torch.nan_to_num(probs.detach().float().cpu(), nan=0.0, posinf=0.0, neginf=0.0)
    safe_probs = torch.clamp(safe_probs, min=0)
    if safe_probs.numel() == 0:
        return 0

    total = float(safe_probs.sum().item())
    if total <= 0 or not np.isfinite(total):
        return int(torch.argmax(safe_probs).item())

    normalized = (safe_probs / total).numpy()
...
class WatermarkConfig:
    ...
    detect_bias_free: bool = True
    detect_biased: bool = True
    bf_weight: float = 0.0  # To be computed
    b_weight: float = 0.0   # To be computed
    min_biased_weight: float = 0.5
    min_delta_kgw: float = 2.0
...
class OptimalHybridWatermark:
    ...
    def __init__(self, vocab_size: int, config: WatermarkConfig):
        ...
        print(
            ...
            f"BF={config.detect_bias_free and config.use_bias_free}, "
            f"B={config.detect_biased and config.use_biased}"
        )
    
    def _compute_optimal_parameters(self):
        """Compute optimal watermark parameters based on Theorem 1"""
        requested_bias_free = self.config.use_bias_free
        requested_biased = self.config.use_biased
        ...
        if requested_bias_free and D_req <= self.config.D_BF_max:
            # Pure bias-free is sufficient
            D_BF_star = D_req
            D_B_star = 0
            print(f"Strategy: Pure bias-free (D_BF={D_BF_star:.6f})")
        elif not requested_bias_free:
            D_BF_star = 0
            D_B_star = min(D_req, self.config.D_B_max)
            print(f"Strategy: Biased only (D_B={D_B_star:.6f})")
        else:
        ...
        if requested_biased and D_B_star > 0:
        ...
        if requested_bias_free and D_BF_star > 0:
            self.config.bf_weight = D_BF_star / (D_BF_star + D_B_star) if D_B_star > 0 else 1.0
        else:
            self.config.bf_weight = 0
        
        if requested_biased and self.config.min_biased_weight > 0:
            min_biased_weight = min(1.0, max(0.0, self.config.min_biased_weight))
            if self.config.b_weight < min_biased_weight:
                print(
                    "Applying hybrid floor: "
                    f"B weight {self.config.b_weight:.3f} -> {min_biased_weight:.3f}, "
                    f"delta >= {self.config.min_delta_kgw:.3f}"
                )
                self.config.b_weight = min_biased_weight
                self.config.delta_kgw = max(self.config.delta_kgw, self.config.min_delta_kgw)
                self.config.bf_weight = (1.0 - min_biased_weight) if requested_bias_free else 0.0
        
        self.config.use_bias_free = requested_bias_free and self.config.bf_weight > 0
        self.config.use_biased = requested_biased and self.config.b_weight > 0
    ...
    def apply_watermark(self, 
        ...
        if self.config.use_bias_free and self.config.bf_weight > 0:
            ...
            bf_logits = torch.log(
                bf_modified.to(logits.device, dtype=logits.dtype) + 1e-10
            ) * temperature
        ...
        if self.config.use_biased and self.config.b_weight > 0:
            # Get green list mask
            green_mask = self.kgw_watermark.get_green_list_mask(context_tokens).to(watermarked_logits.device)
            
            # delta_kgw is already derived from the biased information budget.
            # Multiplying it by b_weight again makes the KGW signal vanish.
            delta_boost = self.config.delta_kgw
    ...
    def detect(self, text: str, tokenizer) -> Dict:
        ...
        if self.config.detect_biased and self.config.use_biased and self.config.b_weight > 0:
            kgw_score = self.kgw_watermark.compute_score(tokens, tokenizer)
            scores.append(('biased', kgw_score, self.config.b_weight))
        
        if self.config.detect_bias_free and self.config.use_bias_free and self.config.bf_weight > 0:
        ...
        return {
            ...
            "score": float(adjusted_z),
            "z_score": float(adjusted_z),
            "p_value": float(p_value),
            "confidence": float(1 - p_value),
            "strategy": strategy,
            "component_weights": {
                name: float(weight) for name, _, weight in scores
            },
            "component_scores": {
                name: {
                    metric: float(value)
...
class HuBiasFreeWatermark:
    ...
    def modify_distribution(self, 
        ...
        safe_probs = probs.detach().float().cpu()
        ...
        if self.method == 'delta':
            ...
            u = rng.uniform(0, 1)
            cumsum = torch.cumsum(safe_probs, dim=-1)
            idx = torch.searchsorted(
                cumsum,
                torch.tensor(u, dtype=cumsum.dtype),
            ).item()
            
            modified = torch.zeros_like(safe_probs)
            modified[min(idx, len(safe_probs)-1)] = 1.0
            
            # Soft blending for robustness
            alpha = 0.8  # Blending factor
            return alpha * modified + (1-alpha) * safe_probs
        
        else:  # gamma reweight
            indices = np.arange(len(safe_probs))
            rng.shuffle(indices)
            
            shuffled = safe_probs[indices]
...
class HybridWatermarkedLLM:
    ...
    def __init__(self,
                 ...
                 hybrid_min_biased_weight: Optional[float] = None,
                 hybrid_min_delta_kgw: Optional[float] = None):
        ...
        env_min_biased_weight = os.environ.get("HYBRID_MIN_BIASED_WEIGHT")
        env_min_delta_kgw = os.environ.get("HYBRID_MIN_DELTA_KGW")
        if hybrid_min_biased_weight is not None:
            config.min_biased_weight = float(hybrid_min_biased_weight)
        elif env_min_biased_weight is not None:
            config.min_biased_weight = float(env_min_biased_weight)
        if hybrid_min_delta_kgw is not None:
            config.min_delta_kgw = float(hybrid_min_delta_kgw)
        elif env_min_delta_kgw is not None:
            config.min_delta_kgw = float(env_min_delta_kgw)
        config.detect_bias_free = _env_bool("HYBRID_DETECT_BIAS_FREE", config.detect_bias_free)
        config.detect_biased = _env_bool("HYBRID_DETECT_BIASED", config.detect_biased)
    ...
    def generate(self,
                ...
                watermark: bool = True,
                detect_watermark: bool = True) -> Tuple[str, Dict]:
        """Generate text with optional hybrid watermarking"""

        debug = os.environ.get("REVIEWER2_DEBUG") == "1"
        ...
        if debug:
            _debug_log(f"[HYBRID] prompt_tokens={len(generated_ids)} max_new_tokens={max_new_tokens} watermark={watermark}")
        ...
        for _ in range(max_new_tokens):
            ...
            # Sample next token
            if temperature > 0:
                probs = torch.softmax(logits / max(temperature, 1e-6), dim=-1).detach().cpu()
                probs = torch.nan_to_num(probs, nan=0.0, posinf=0.0, neginf=0.0)
                next_token = _safe_choice_from_probs(probs)
            else:
            ...
            next_input_ids = torch.tensor([[next_token]], dtype=torch.long, device=self.device)
        ...
        # Detect watermark
        detection = self.watermark.detect(generated_text, self.tokenizer) if detect_watermark else {}

        if debug:
            _debug_log(f"[HYBRID] completion_tokens={len(generated_ids) - input_length} text_chars={len(generated_text)}")
